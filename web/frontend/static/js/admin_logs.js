...
function renderTable(logs, tbody) {
    ...
    logs.forEach(log => {
        ...
        let timeClass = "text-muted", textClass = "";

        if (log.level === 'CRITICAL') { rowClass = "table-dark"; badgeClass = "bg-light text-dark"; timeClass = "text-light"; textClass = "text-light"; }
        ...
        if (textClass) tr.classList.add(textClass);
        tr.innerHTML = `
                <td class="text-nowrap small ${timeClass}">${timeStr}</td>
