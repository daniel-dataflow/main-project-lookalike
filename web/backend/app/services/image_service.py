...
class ImageService:
    ...
    async def process_and_upload_thumbnail(self, file: UploadFile, user_id: str) -> dict:
        ...
        metadata = self._get_image_metadata(content)
        thumb_bytes = self._create_thumbnail_bytes(
        ...
        # HDFS 업로드
        hdfs_ok = await self._upload_bytes_to_hdfs(thumb_bytes, hdfs_thumb_path)
    ...
    def _create_thumbnail_bytes(
        self, image_bytes: bytes, size: int = 150, quality: int = 85
    ) -> bytes:
    ...
    async def _upload_bytes_to_hdfs(self, data: bytes, hdfs_path: str) -> bool:
        """
    ...
    async def read_thumbnail_from_hdfs(self, hdfs_path: str) -> bytes | None:
