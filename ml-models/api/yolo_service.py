...
class YoloDetector:
    ...
    """
    def __init__(self, model_name: str = "../yolo/yolo_weights.pt"):
    ...
    def load(self):
        try:
            ...
            weight_path = os.path.abspath(os.path.join(os.path.dirname(__file__), self.model_name))
            logger.info(f"Custom YOLO 모델({weight_path}) 로드 중...")
            
            if not os.path.exists(weight_path):
                logger.error(f"YOLO 가중치 파일을 찾을 수 없습니다: {weight_path}")
                return
    ...
    def detect_boxes(self, pil_img: Image.Image) -> list:
        ...
        (기본 COCO 모델 사용 시 person, backpack, umbrella, tie, suitcase 등이 있지만,
         ...
         여기서는 기본 yolo11n.pt를 활용하되, 프론트에서 크롭 확인하도록 범용 BBox를 던집니다.)
        """
