...
def yolo_reorganize_dedup_upsert(
    ...
    for row in best_by_product_id.values():
        ...
        safe_brand = str(row.get("brand") or "unknown")
        safe_gender = str(row.get("gender") or "unknown")
        safe_category = str(row.get("category") or "unknown")
        safe_code = str(row.get("product_code") or "unknown")
