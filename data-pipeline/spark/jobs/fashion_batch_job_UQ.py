...
from pyspark.sql.functions import (lit, col, current_timestamp, concat, format_string, 
                                   ...
                                   regexp_extract, to_json, element_at)
...
# 하둡 접속 정보
HDFS_BASE = "hdfs://namenode-main:9000"        # Spark RDD 읽기용 RPC 포트
HDFS_WEB_URL = "http://namenode-main:9870"     # 이미지 직접 업로드용 WebHDFS 포트
...
if not row:
    print(f"✨ {BRAND_NAME.upper()} sequence not found. Registering new brand in DB...")
...
# 파일명(예: uniqlo_men_outer_E12345_09.json)에서 성별과 카테고리 추출
raw_df = raw_df \
...
pg_data = processed_df.select(
    col("product_id"),
    coalesce(col("display_goods_no"), col("goodsNo")).alias("model_code"),
...
mongo_data = processed_df.select(
    ...
    lit(BRAND_NAME.upper()).alias("brand_name"),
    col("goodsNm").alias("prod_name"),
    to_json(col("goodsMaterial")).alias("detail_desc"), # 상세 설명 및 소재 정보
    coalesce(col("price").cast("int"), lit(0)).alias("base_price"),
    col("img_hdfs_path"),
    current_timestamp().alias("create_dt")
)
...
try:
    ...
    print(f"🚨 하둡 클라이언트 연결/이미지 전송 실패: {e}")
