...
#  📦 테스트 데이터 삽입 스크립트
# ============================================
# 의류 상품 10개 + 각 상품당 5개 쇼핑몰 가격 데이터 생성
...
# 1. Products 테이블에 의류 상품 10개 삽입
echo "1️⃣  Products 테이블에 의류 상품 10개 삽입..."
docker exec ${CONTAINER_NAME} psql -U ${DB_USER} -d ${DB_NAME} << 'EOF'
INSERT INTO products (model_code, brand_name, prod_name, base_price, category_code, img_hdfs_path)
VALUES
  ('UT-2024-WH', 'UNIQLO', '프리미엄 코튼 반팔 티셔츠 - 화이트', 19900, '상의', 'https://image.uniqlo.com/UQ/ST3/AsianCommon/imagesgoods/422990/item/goods_01_422990.jpg'),
  ('DM-2024-BL', 'ZARA', '슬림핏 데님 자켓 - 블루', 89900, '아우터', 'https://static.zara.net/photos///2024/V/0/1/p/4661/450/400/2/w/750/4661450400_1_1_1.jpg'),
  ('SL-2024-BK', 'H&M', '와이드핏 슬랙스 - 블랙', 39900, '하의', 'https://www2.hm.com/content/dam/hm/about/2024/01/0001/0001-3x2/3x2-Trousers-Black.jpg'),
  ('HD-2024-GY', 'Nike', '에센셜 후드 집업 - 그레이', 69900, '아우터', 'https://static.nike.com/a/images/t_PDP_1280_v1/f_auto,q_auto:eco/61734ec7-dad8-40f3-9b95-c7500939150a/sportswear-club-fleece-mens-full-zip-hoodie-hDv5Md.png'),
  ('SK-2024-NV', 'SPAO', '미니 데님 스커트 - 네이비', 29900, '하의', 'https://image.spao.com/image/goods/2024/01/SPSK4TG01_BL1_1.jpg'),
  ('CT-2024-BE', 'MUJI', '오가닉 코튼 가디건 - 베이지', 49900, '상의', 'https://img.muji.net/img/item/4550512681861_1260.jpg'),
  ('JN-2024-BL', 'Levis', '501 오리지널 진 - 블루', 119000, '하의', 'https://lsco.scene7.com/is/image/lsco/005010101-front-pdp?fmt=jpeg&qlt=70&resMode=sharp2&fit=crop,1&op_usm=0.6,0.6,8&wid=2000&hei=1840'),
  ('SW-2024-WH', 'Adidas', '트레포일 로고 맨투맨 - 화이트', 59900, '상의', 'https://assets.adidas.com/images/w_600,f_auto,q_auto/8a2871d9c4b44e0ab5f0af3d00e8ca38_9366/Trefoil_Essentials_Crewneck_Sweatshirt_White_IU2664_01_laydown.jpg'),
  ('DR-2024-BK', 'ZARA', '미디 A라인 원피스 - 블랙', 79900, '원피스', 'https://static.zara.net/photos///2024/V/0/1/p/2731/144/800/2/w/750/2731144800_1_1_1.jpg'),
  ('SN-2024-WH', 'Converse', '척테일러 올스타 로우 - 화이트', 69000, '신발', 'https://www.converse.com/dw/image/v2/BCZC_PRD/on/demandware.static/-/Sites-cnv-master-catalog/default/dw3c8c3e3e/images/a_107/M7652_A_107X1.jpg');
EOF

if [ $? -eq 0 ]; then
    echo "   ✅ Products 테이블 삽입 완료"
else
    echo "   ❌ Products 테이블 삽입 실패"
    exit 1
fi
echo ""

# 2. Naver_prices 테이블에 각 상품당 5개 쇼핑몰 가격 삽입
echo "2️⃣  Naver_prices 테이블에 쇼핑몰 가격 데이터 삽입..."
docker exec ${CONTAINER_NAME} psql -U ${DB_USER} -d ${DB_NAME} << 'EOF'
-- 먼저 삽입된 product_id를 확인하여 변수에 저장
DO $$
DECLARE
...
if [ $? -eq 0 ]; then
    echo "   ✅ Naver_prices 테이블 삽입 완료"
else
    echo "   ❌ Naver_prices 테이블 삽입 실패"
    exit 1
fi
echo ""

# 3. Product_features 테이블에 상품 설명/특징 삽입
...
if [ $? -eq 0 ]; then
    echo "   ✅ Product_features 테이블 삽입 완료"
else
    echo "   ❌ Product_features 테이블 삽입 실패"
    exit 1
fi
echo ""
...
# 4. 삽입된 데이터 확인
echo "4️⃣  삽입된 데이터 확인..."
PRODUCT_COUNT=$(docker exec ${CONTAINER_NAME} psql -U ${DB_USER} -d ${DB_NAME} -t -c "SELECT COUNT(*) FROM products WHERE model_code LIKE '%-2024-%';")
PRICE_COUNT=$(docker exec ${CONTAINER_NAME} psql -U ${DB_USER} -d ${DB_NAME} -t -c "SELECT COUNT(*) FROM naver_prices np JOIN products p ON np.product_id = p.product_id WHERE p.model_code LIKE '%-2024-%';")
FEATURE_COUNT=$(docker exec ${CONTAINER_NAME} psql -U ${DB_USER} -d ${DB_NAME} -t -c "SELECT COUNT(*) FROM product_features pf JOIN products p ON pf.product_id = p.product_id WHERE p.model_code LIKE '%-2024-%';")

echo "   📊 Products: ${PRODUCT_COUNT}개"
echo "   📊 Naver_prices: ${PRICE_COUNT}개"
echo "   📊 Product_features: ${FEATURE_COUNT}개"
echo ""
...
echo ""
echo "  삽입된 데이터:"
echo "    ✅ 의류 상품: ${PRODUCT_COUNT}개"
echo "    ✅ 쇼핑몰 가격: ${PRICE_COUNT}개"
echo "    ✅ 상품 설명: ${FEATURE_COUNT}개"
echo "============================================"
