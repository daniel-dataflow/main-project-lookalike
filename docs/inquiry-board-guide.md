...
│   - 본인 문의글만 열람         - 모든 문의글 열람      │
│   - 작성/수정/삭제(답변 전)    - 답변 작성/수정        │
│   - 답변 확인                 - 상태 필터링           │
└───────────┬──────────────────────┬────────────────┘
            │ API 호출              │ API 호출
            ▼                      ▼
...
│   ├── [유저 API]  /api/inquiries/*                  │
│   └── [관리자 API] /api/inquiries/admin/*            │
│                                                    │
│   routers/auth.py                                  │
...
| **테이블** | 기존 `posts`/`comments` 구조 참고하여 `inquiry_board` | 일관된 스키마 패턴 유지 |
| **답변 방식** | 별도 댓글 테이블이 아닌 **문의글에 answer 컬럼** 포함 | 1:1 문의-답변 구조에 적합 |
...
│
scripts/
└── apply_db_changes.sh         # DB 마이그레이션 스크립트
...
```
[유저 문의 작성] → status: 'pending' (대기중)
                       ↓
            [관리자 답변 작성]
                       ↓
              status: 'answered' (답변완료)
              + answer, answered_by, answered_at 업데이트
```
...
`docker-compose.yml`의 `init-db` 서비스에 모든 `CREATE TABLE IF NOT EXISTS`가 포함되어 있습니다.
